...
function originalToJSLibFigure1(figure) {
  ...
  var get_parameter_values = function (param) {
    if (param.name === 'who') {
      ...
      return true;              // all old pass-throughs let the other couple pass on the left
...
function unpackMoveName(oldFigure) {
  ...
  var oldMove_ends_in_left = oldMove.match(/_left(_shoulder)?$/);
  var oldMove_ends_in_right = oldMove.match(/_right(_shoulder)?$/);
...
function jsLibFigureToOriginal1(figure) {
  ...
  var restored_note = figure.note ? dropFormationSuffixFromNote(figure.note).trim() : '';
...
function findOldMove(lf_figure) {
  ...
  if (oldAtomicMoves[underscore_lf_move]) {
    ...
    if (index < 0) { throw_up('can not find a turn, hand, shoulder, or side for '+lf_move); }
