...
function jsLibFigureToOriginal1(figure) {
  ...
  var circling = extractCircling(params, param_vals);
  if (circling) { h.circling = circling; }
  if (extractBal(params, param_vals)) { h.bal = true; }
  if (figure.notes) { h.notes = figure.notes; }
  return h;
}
...
function extractCircling(params, param_vals) {
...
function testConverters(figures) {
  return figures.map(function(figure) {
    return [figure, jsLibFigureToOriginal1(originalToJSLibFigure1(figure))];
  });
}
