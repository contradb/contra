...
function originalToJSLibFigure1(figure) {
  ...
  var get_parameter_values = function (param) {
    if (param.name === 'who') {
      ...
      return undefined;
    } else if (param.name === 'diag') {
      return (figure.notes && parse_string_for_diagonal(figure.notes)) || '';
...
function jsLibFigureToOriginal1(figure) {
  ...
  var restored_note = custom_text + extractDiagonal(params, param_vals) + (figure.note ? dropFormationSuffixFromNote(figure.note).trim() : '');
...
function extractDiagonal(params, param_vals) {
  var index = find_parameter_index_by_name('diag', params);
  return (index >= 0) ? param_vals[index] : '';
}
