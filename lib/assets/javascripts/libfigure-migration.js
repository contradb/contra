...
function originalToJSLibFigure1(figure) {
  ...
  var get_parameter_values = function (param) {
    if (param.name === 'who') {
      ...
      var diagonal_note = figure.notes && parse_string_for_diagonal(figure.notes);
      return diagonal_note || defaultSetDirection(move);
    } else if (param.name === 'ender') {
      return '';
  ...
  if (figure.notes && figure_has_no_custom_text_parameters && !parse_string_for_diagonal(figure.notes)) { h.note = figure.notes; }
...
function unpackMoveName(oldFigure) {
  ...
  var move = _decode_pull_by(oldFigure) || _unpack_move_name[oldMove] || oldMove.replace(/_/g, ' ');
  var h = {move: move}
...
function _decode_pull_by(oldFigure) {
  var oldMove = oldFigure.move;
  if (oldMove !== 'pull_by_left' && oldMove !== 'pull_by_right') {
    return null;
  } else if (oldFigure.who === 'everybody') {
    return 'pull by for 4';
  } else {
    return 'pull by for 2';
  }
}
...
// sometimes JSLibFigure figures have parameters that original did not
// and the new parameters have known defaults,
// so fill them in.
function default_missing_parameters(figure) {
  var formals = parameters(figure.move);
  var actuals = figure.parameter_values;
  if (formals.length != actuals.length) {
    throw_up("arity mismatch on default_missing_parameters(#<"+figure.move+" ...>)");
  }
  for (var i = 0; i<formals.length; i++) {
    var actual_needs_defaulting = actuals[i] === undefined || actuals[i] === null;
    if (actual_needs_defaulting) {
      actuals[i] = formals[i].value;
    }
  }
}

var left_diagonal_regexp = /^(along )?left diag[oa]n[ao]l$/i;
var right_diagonal_regexp = /^(along )?right diag[oa]n[ao]l$/i;

function parse_string_for_diagonal(text) {
  if (text.match(left_diagonal_regexp)) {
    return 'left diagonal';
  } else if (text.match(right_diagonal_regexp)) {
...
function jsLibFigureToOriginal1(figure) {
  ...
  var diagonal_note = extractDiagonal(params, param_vals) || ''
  var figure_note = figure.note ? dropFormationSuffixFromNote(figure.note).trim() : ''
  var restored_note = custom_text + diagonal_note + figure_note;
...
function extractDiagonal(params, param_vals) {
  var setDirection = extractSetDirection(params, param_vals);
  if (setDirection == 'left diagonal' || setDirection == 'right diagonal') {
    return setDirection;
  } else {
    return null;
  }
...
function extractSetDirection(params, param_vals) {
  var index = find_parameter_index_by_name('dir', params);
  return (index >= 0) ? param_vals[index] : null;
}
...
var oldLeftRightMoves = {
  ...
  pull_by_for_4: ['pull_by_right', 'pull_by_left'],
  pull_by_for_2: ['pull_by_right', 'pull_by_left'],
...
function defaultSetDirection(move) {
  var ps = parameters(move);
  var i = find_parameter_index_by_name('dir', ps);
  return ps[i].value;
}
