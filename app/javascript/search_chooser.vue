<template>
  <div>
    <select
      v-if="chooserSelectOptions()"
      v-model="value"
      class="form-control chooser-argument"
    >
      <option
        v-for="option in chooserSelectOptions()"
...
export default {
  ...
  computed: {
    value: {
      ...
      set: function(value) {
        this.$store.dispatch("setParameter", {
          path: this.path,
    ...
    chooserName: function() {
      return this.formalParameter.ui.name
    },
    uniqueName: function() {
      return "cb-" + this.path.join("-") + "-" + this.parameterIndex
    }, // assumes only 1 of these per page
  },
  methods: {
    chooserSelectOptions: function() {
      if (
        this.chooserName === "chooser_revolutions" ||
        this.chooserName === "chooser_places"
      )
        return [
          "*",
          ...LibFigure.anglesForMove(this.move).map(angle => [
            angle.toString(),
            LibFigure.degreesToWords(angle, this.move),
          ]),
        ]
      else return this.$store.getters.selectChooserNameOptions[this.chooserName]
    },
