<template>
  <div class="figure-filter">
    ...
    >
      <option v-for="{ term, substitution } in moveMenu" v-bind:value="term">{{
        substitution
      }}</option>
    </select>
    <span v-if="op === 'figure' && move !== '*'"
      ><bootstrap-toggle
        v-model="ellipsis"
        :options="{ on: 'more', off: 'less' }"
    /></span>

    <span v-if="left !== undefined"
      ><SearchExEditor v-bind:path="path.concat(0)" v-bind:lisp="left.toLisp()"
    /></span>

    <select
      v-if="comparison !== undefined"
      v-model="comparison"
      class="figure-filter-count-comparison form-control"
    >
      <option>≥</option>
      <option>≤</option>
      <option>&gt;</option>
    ...
    <span v-if="right !== undefined"
      ><SearchExEditor v-bind:path="path.concat(1)" v-bind:lisp="right.toLisp()"
    /></span>

    <select
      v-if="number !== undefined"
      v-model="number"
      class="figure-filter-count-number form-control"
    >
      <option
        v-for="i in [
          0,
    ...
    <select
      ...
      class="figure-filter-count-number form-control"
    >
      <option v-for="tagName in this.$store.state.tagNames">{{
        tagName
      }}</option>
    </select>

    <select
      v-if="formation !== undefined"
      v-model="formation"
      class="figure-filter-formation form-control"
    >
    ...
    <div class="btn-group">
      <button
        type="button"
        class="btn btn-default dropdown-toggle figure-filter-menu-hamburger"
      ...
      <ul class="dropdown-menu figure-filter-menu">
        ...
        <li v-if="deleteEnabled()">
          <a class="figure-filter-menu-delete" v-on:click="clickDelete()">X</a>
        </li>
      </ul>
    </div>
    <table v-if="ellipsis" class="figure-filter-accordion">
      <tr
        v-for="(formalParameter, parameterIndex) in formalParameters(move)"
        class="chooser-row"
      >
    ...
    <ul v-if="subordinateSubexpressionLayout">
      <li v-for="(subEx, index) in searchEx.subexpressions">
        <SearchExEditor
          v-bind:path="path.concat(index)"
          v-bind:lisp="subEx.toLisp()"
        />
      </li>
    </ul>
...
export default {
  ...
  computed: {
    ...
    parentSearchEx: function() {
      if (0 === this.path.length) return null
      else {
    ...
    moveMenu: function() {
      return [{ term: "*", substitution: "any figure" }].concat(
        LibFigure.moveTermsAndSubstitutionsForSelectMenu(
          this.$store.state.dialect
        )
      )
    },
    op: {
    ...
    left: {
      get: function() {
        return this.searchEx.left
      },
    },
    right: {
      get: function() {
        return this.searchEx.right
      },
    },
  ...
  methods: {
    ...
    deleteEnabled: function() {
      return (
        this.path.length &&
        this.parentSearchEx.subexpressions.length >
          this.parentSearchEx.minSubexpressions()
      )
    },
  },
  components: {
