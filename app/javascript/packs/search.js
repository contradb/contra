...
function selectChooserNameOptions(dialect) {
  return {
    ...
    ...LibFigure.dancerChooserNames().reduce((acc, chooserName) => {
      acc[chooserName] = ["*"].concat(
        LibFigure.dancerMenuForChooser(LibFigure.chooser(chooserName)).map(
          dancers => [dancers, LibFigure.dancerMenuLabel(dancers, dialect)]
        )
      )
      return acc
    }, {}),
...
$(document).ready(function() {
  ...
  // duplicated code in welcome.js
  var dataTable = $("#dances-table-vue").DataTable({
    processing: true,
    serverSide: true,
    ajax: {
