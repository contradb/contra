...
function findDanceHashById(dance_id, dances) {
  if (dance_id)
    ...
    for (var i = 0; i < dances.length; i++)
      if (dance_id == dances[i].id) return dances[i]
  return null
}
...
function deleteSelectetdActivities(activities) {
  ...
  while (i < activities.length)
    if (activities[i].checked) activities.splice(i, 1)
    else i++
  return
}

function selectAllActivities(activities) {
  for (var i = 0; i < activities.length; i++) activities[i].checked = true
  return
}

function deselectAllActivities(activities) {
  for (var i = 0; i < activities.length; i++) activities[i].checked = false
  return
}

// paste an array onto one end of another array
function pasteArray(modified, src) {
  for (var i = 0; i < src.length; i++) modified.push(src[i])
}
...
function moveSelectedActivitiesToTop(activities) {
  ...
  for (var i = 0; i < activities.length; i++)
    activities[i].checked
      ? tops.push(activities[i])
      : bottoms.push(activities[i])
...
function moveSelectedActivitiesUpNIndicies(activities, n) {
  ...
  activities.length = 0 // clear activities destructively!
  for (var i = 0; i < tmp.length; i++) activities.push(tmp[i][1])
  return
}
...
function checkedActivityCount(activities) {
  acc = 0
  for (i = 0; i < activities.length; i++) if (activities[i].checked) acc++
  return acc
}
...
function dragEnterOver(ev) {
  ...
  // highlight new
  if (!tt.isEqualNode(dragInitiator))
    // global variable
...
function eventSetDataTransfer(ev, value) {
  ...
  try {
    dataTransfer.setData("contradbActivity", value)
  } catch (e) {
    console.log(
      'warning: drag and drop setData failed.\n"' +
        e +
...
function eventGetDataTransfer(ev) {
  ...
  try {
    ...
    if (txt && txt.length > 0) return txt
      ...
      throw new Error("Got null or nullstring Data, trying another approach...")
  } catch (e) {
    console.log(
      'warning: drag and drop getData failed.\n"' +
        e +
        '"\nSmells like a Microsoft Browser.\n' +
        "Retrying with riskier parameters..."
    )
    ...
    if (txt && txt.length > 0) return txt
      ...
      throw new Error(
        "Also can't get meaningful text. I give up! See above warning"
      )
  }
}
function eventDataTransferProperty(ev) {
  if (ev.dataTransfer && ev.dataTransfer.setData) return ev.dataTransfer
  else if (
    ev.originalEvent &&
...
function drop(ev) {
  ...
  $(".activity-row")
    .removeClass("contra-drop-enthusiastic")
    .removeClass("contra-drag-origin")
...
function activityRowIndex(id) {
  ...
  if (id.slice(0, prefix.length) == prefix)
    ...
    throw new Error(
      "called activityRowIndex on something that's not an activity-row id string. It looks like this: " +
        whatsThis(id)
    )
}
...
;(function() {
  ...
  var scopeInit = function($scope) {
    ...
    $scope.getActivities = function() {
      return activities_ctrl.activities
    }
  }
