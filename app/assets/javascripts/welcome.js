$(document).ready(function() {
  ...
  function installGenericFilterEventHandlers(selector) {
    selector
      .find(".figure-filter-op")
      .off("change")
      .change(filterOpChanged) // do not double-install filterOpChanged
    selector
      .find(".figure-filter-add")
      .off("click")
      .click(clickFilterAddSubfilter) // do not double-install clickFilterAddSubfilter
    selector
      .find(".figure-filter-remove")
      .off("click")
      .click(filterRemoveSubfilter) // do not double-install filterRemoveSubfilter
  ...
  var removeButtonHtml =
    "<button class='figure-filter-remove'><span class='glyphicon glyphicon-remove'></span></button>"

  if (!Array.prototype.forEach) {
    throw "I was expecting Array.forEach to be defined"
  }
  if (!Array.prototype.map) {
    throw "I was expecting Array.map to be defined"
  }

  var figureMoveHtml =
    "<select class='figure-filter-move form-control'><option value='*' selected=true>any figure</option>"
  moveTermsAndSubstitutionsForSelectMenu(dialect).forEach(function(move) {
    figureMoveHtml +=
      '<option value="' + move.term + '">' + move.substitution + "</option>"
  })
  ...
  var formationSelectHtml =
    "<select class='figure-filter-formation form-control'>" +
    [
      "improper",
      "Becket *",
      "Becket cw",
  ...
  function filterOpChanged(e) {
    ...
    if (hasNoAddButton && actualSubfilterCount < maxParameterCount(op)) {
      ...
      !hasNoAddButton &&
      actualSubfilterCount >= maxParameterCount(op)
    ) {
      ...
      subfilters.each(function() {
        ...
        if (0 === $subfilter.children(".figure-filter-remove").length) {
          ...
          if ($subfilter.children(".figure-filter").length > 0) {
            ...
            $subfilter
              .children(".figure-filter-end-of-subfigures")
              .before(removeButton)
          }
        }
      })
  ...
  function makeFigureFilterMoveSelect(filter) {
    return $(figureMoveHtml).change(function() {
      ...
      var defaultParameterValues =
        move != "*" && isAlias(move) ? aliasFilter(move) : null
  ...
  function makeFigureFilterEllipsisButton(filter) {
    return $(
      "<button class='btn btn-default figure-filter-ellipsis'>...</button>"
    ).click(clickEllipsis)
  }
  ...
  function addCountFilterConstellation(filter) {
    filter
      ...
      .after(
        $(
          '<select class="figure-filter-count-number form-control"><option>0</option><option>1</option><option selected>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option></select>'
        ).change(updateQuery)
      )
      .after(
        $(
          '<select class="figure-filter-count-comparison form-control"><option>≥</option><option>≤</option><option>&gt;</option><option>&lt;</option><option>=</option><option>≠</option></select>'
        ).change(updateQuery)
      )
  }
  ...
  function chooserRadioButtons(chooser_name, options, opt_inline) {
    if (opt_inline === undefined) {
      opt_inline = options.length <= 3
    }
  ...
  function chooserSelect(chooser_name, options) {
    ...
    chooserNameToFilterHtml[chooser_name] = function(move) {
      ...
      return (
        '<select class="form-control chooser-argument">' +
        htmls.join() +
        "</select>"
      )
    }
  ...
  chooserNameToFilterHtml["chooser_revolutions"] = chooserNameToFilterHtml[
    "chooser_places"
  ] = function(move) {
...

  {
    ...
    for (var i = 0; i < dcs.length; i++) {
      var substituter = function(dancers) {
        return [dancers, dancerMenuLabel(dancers, dialect)]
      }
      chooserSelect(
        dcs[i],
        ["*"].concat(
          dancerCategoryMenuForChooser(chooser(dcs[i])).map(substituter)
        )
      )
    }
  }

  chooserRadioButtons("chooser_spin", [
    "*",
    [true, "clockwise"],
    [false, "ccw"],
  ])
  chooserRadioButtons("chooser_left_right_spin", [
    "*",
    [true, "left"],
    [false, "right"],
  ])
  chooserRadioButtons("chooser_right_left_hand", [
    "*",
    [false, "left"],
    [true, "right"],
  ])
  chooserRadioButtons("chooser_right_left_shoulder", [
    "*",
    [false, "left"],
    [true, "right"],
  ])
  ...
  chooserSelect(
    "chooser_star_grip",
    ["*"].concat(
      wristGrips.map(function(grip) {
        return grip === "" ? ["", "unspecified"] : grip
      })
    )
  )

  chooserSelect("chooser_march_facing", [
    "*",
    "forward",
    "backward",
  ...
  chooserSelect("chooser_set_direction", [
    "*",
    ["along", "along the set"],
  ...
  chooserSelect("chooser_set_direction_acrossish", [
    "*",
    ["across", "across the set"],
    "right diagonal",
    "left diagonal",
  ])
  chooserSelect("chooser_set_direction_grid", [
    "*",
    ["along", "along the set"],
    ["across", "across the set"],
  ])
  chooserSelect("chooser_set_direction_figure_8", [
    "*",
    "",
    "above",
    "below",
  ...
  chooserSelect("chooser_gate_direction", [
    "*",
    ["up", "up the set"],
    ["down", "down the set"],
    ["in", "into the set"],
  ...
  chooserSelect("chooser_slice_return", [
    "*",
  ...
  chooserSelect("chooser_all_or_center_or_outsides", [
    "*",
    "all",
    "center",
    "outsides",
  ...
  chooserSelect("chooser_zig_zag_ender", [
    "*",
    ["", "none"],
    ["ring", "into a ring"],
  ...
  chooserRadioButtons("chooser_go_back", [
    "*",
    [true, "forward &amp; back"],
    [false, "forward"],
  ])
  chooserRadioButtons("chooser_give", [
    "*",
    [true, "give &amp; take"],
    [false, "take"],
  ])
  chooserRadioButtons("chooser_half_or_full", [
    "*",
    [0.5, "half"],
    [1.0, "full"],
  ])
  ...
  function doesChooserFilterUseSelect(chooser) {
    if (!chooser.name) {
      throw_up("expected a chooser, got " + JSON.stringify(chooser))
    }
  ...
  function populateAccordionForMove(accordion, move, optionalParameterValues) {
    ...
    formals.forEach(function(formal, index) {
      var html_fn =
        chooserNameToFilterHtml[formal.ui.name] ||
        function() {
          return "<div>" + formal.name + "</div>"
        }
      ...
      var label = $(
        '<tr class="chooser-row"><td class="chooser-label-text">' +
          parameterLabel(move, index) +
          "</td></tr>"
      )
  ...
  function filterAddSubfilter(parentFilter) {
    // caller is responsible to updateQuery() when done
    var childFilter = $(filterHtml)
    installGenericFilterEventHandlers(childFilter)
    addFigureFilterMoveConstellation(childFilter)
    childFilter.insertBefore(
      parentFilter.children(".figure-filter-end-of-subfigures")
    )
  ...
  function buildFigureQuery(figure_filter) {
    ...
    if (op === "figure") {
      ...
      formals.forEach(function(formal, i) {
        const $chooser = $(
          figure_filter
            .children(".figure-filter-accordion")
            .find(".chooser-row")[i]
        ).find(".chooser-argument")
      ...
      if (op === "count") {
        filter.push(
          figure_filter.children(".figure-filter-count-comparison").val()
        )
  ...
  function buildDOMtoMatchQuery(query) {
    ...
    switch (op) {
      case "figure":
        ...
        if (query.length > 2) {
          // ... was clicked
          figureFilter
            .children(".figure-filter-ellipsis")
            .toggleClass("ellipsis-expanded")
      ...
      case "count":
        ...
        figureFilter
          .children(".figure-filter-end-of-subfigures")
          .before(buildDOMtoMatchQuery(subfilter))
        break
      case "progression":
        break
      default:
        if (n_ary(op) || unary(op)) {
          if ("count" === op) {
            throw_up("count is a special case supposedly already handled")
          }
          for (var i = 1; i < query.length; i++) {
            figureFilter
              .children(".figure-filter-end-of-subfigures")
              .before(buildDOMtoMatchQuery(query[i]))
          }
        } else {
          throw new Error("unknown operator: " + op)
        }
    ...
    if (
      figureFilter.children(".figure-filter").length < maxParameterCount(op)
    ) {
  ...
  function insertVisToggles(dataTable) {
    ...
    $("#dances-table thead th").each(function(index) {
      var link = $(
        "<button class='toggle-vis toggle-vis-active btn btn-xs' href='javascript:void(0)'>" +
          $(this).text() +
          "</button>"
      )
      link.click(function() {
  ...
  function toggleColumnVisForResolution(dataTable, width) {
    var screenColumns =
      width < 400
        ...
        ? narrowScreenColumns
        : wideScreenColumns
    if (recentlySeenColumns === screenColumns) {
      return
    }
  ...
  // duplicated code in search.js
  var dataTable = $("#dances-table").DataTable({
    processing: true,
    serverSide: true,
    ajax: {
      ...
      data: function(d) {
        ...
        d.figureQuery = arrayToIntHash(
          JSON.parse($("#figure-query-buffer").val())
        )
      },
    },
    pagingType: "full_numbers",
    dom:
      'f<"table-column-vis-wrap"<"table-column-vis-toggles">>t<"row"<"col-sm-6 col-md-3"i><"col-sm-6 col-md-3"l>>pr',
    language: {
      searchPlaceholder: "filter by title, choreographer, and user",
    },
    columns: [
      // mapped, in order, to <th> ordering in the html.erb
      { data: "title" },
