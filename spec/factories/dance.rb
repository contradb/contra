FactoryGirl.define do
  ...
  factory :dance_with_a_down_the_hall, class: Dance do
    ...
    choreographer { FactoryGirl.create(:choreographer) }
    start_type 'improper'
    figures_json {"[{'parameter_values':['everyone', 'all', '#{march_facing}','#{down_the_hall_ender}',8],'move':'down the hall'}]".gsub("'", '"')}
  end
  ...
  factory :dance_with_a_hey, class: Dance do
    ...
    choreographer { FactoryGirl.create(:choreographer) }
    start_type 'improper'
    figures_json {%([{"parameter_values":["gentlespoons","",true,#{hey_length.to_json},"across",false,false,false,false,8],"move":"hey"}])}
  end
