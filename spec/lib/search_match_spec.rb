...
describe SearchMatch do
  ...
  describe '==' do
    ...
    it do
      expect(SearchMatch.new(4, 2, 8)).to_not eq(SearchMatch.new(4, 8))
    end
  end

  describe 'abut' do
    it 'returns a new search_match on success' do
      expect(SearchMatch.new(4,8).abut(SearchMatch.new(5,8))).to eq(SearchMatch.new(4,2,8))
    end
    ...
    it 'returns nil when they overlap' do
      expect(SearchMatch.new(4,5,8).abut(SearchMatch.new(5,6,8))).to eq(nil)
    end

    it 'wraps' do
      result = SearchMatch.new(4,5).abut(SearchMatch.new(0,5))
      expect(result).to eq(SearchMatch.new(4,2,5))
    ...
    it 'self-wraps' do
      result = SearchMatch.new(0,4,4).abut(SearchMatch.new(0,4,4))
