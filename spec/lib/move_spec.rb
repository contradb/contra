...
RSpec.describe Move do
  ...
  describe ".following_mdtab" do
    it "works" do
      ...
      expect(Move.following_mdtab([box_the_gnat], 'swing')).to eq(expected)
    end
    ...
    it "works off the end of the array" do
      ...
      expect(Move.following_mdtab([dance], 'circle')).to eq(expected)
    end
    ...
    it "works with two dances with partially overlapping moves" do
      ...
      expect(Move.following_mdtab([box_the_gnat, call_me], 'swing')).to eq(expected)
    end
  end
  ...
  describe ".preceeding_mdtab" do
    it "works" do
      ...
      expect(Move.preceeding_mdtab([box_the_gnat], 'swing')).to eq(expected)
    end

    it "works off the end of the array" do
      box_the_gnat = FactoryGirl.build(:box_the_gnat_contra)
      expected = {'chain'=>Set.new([box_the_gnat])}
      expect(Move.preceeding_mdtab([box_the_gnat], 'box the gnat')).to eq(expected)
    end
